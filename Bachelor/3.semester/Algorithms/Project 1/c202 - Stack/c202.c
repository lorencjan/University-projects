...
*/
	//osetreni nuloveho vstupu
	if (s == NULL)
	{
		stackError(SERR_INIT);
		return;
	}
	//nastavim vrchol zasobniku -> je prazdny, takze -1 viz. zadani
	s->top = -1;
}
...
** funkci stackEmpty.
*/
	//overeni prazdneho zasobniku
	if (stackEmpty(s))
	{
		stackError(SERR_TOP);
		return;
	}
	//prirazeni vrcholu zasobniku
	*c = s->arr[s->top];
}
...
*/
	...
	if (!stackEmpty(s))
	{
		//predpokladam, ze kdyz je to staticke pole, na garbage hodnoty se moc nehraje,
		//stejne je pamet naalokovana at uz to prepisu nebo ne, tak jenom posunu vrchol zasobniku
		s->top--;
		;
	}
}
...
** funkci stackFull.
*/
	//overeni plneho zasobniku
	if (stackFull(s))
	{
		stackError(SERR_PUSH);
		return;
	}
	//pridani hodnoty na vrchol
	s->arr[++s->top] = c;
}
