...
**/
	...
	if(!(*RootPtr)) //strom je prazdny, novy list bude root
	{
		*RootPtr = newNode;
	}
	else
	{   
		//vyhledam pozici pro vlozeni / ignoruju, pokud se klic nasel
		tBTNodePtr aux = *RootPtr;
		tBTNodePtr auxParent;
		while(aux)
		{
			auxParent = aux;
			if(aux->Cont < Content)
		...
		//priradim otci prazdne promenne novy list
		if(auxParent->Cont < Content)
			auxParent->RPtr = newNode;
		else if(auxParent->Cont > Content)
			auxParent->LPtr = newNode;
	}
...
**/
	...
	while(!SEmptyP(&s))
	{
		tBTNodePtr aux = STopPopP(&s);
		Leftmost_Preorder(aux->RPtr, &s);
	}
}
...
**/
	...
	{
		if(STopPopB(&stackB)) //pokud jde zleva
		{
			SPushB(&stackB, FALSE);
			tBTNodePtr ptr = STopPopP(&stackP);
			SPushP(&stackP, ptr); //vratime hodnotu na zasobnik, pouze sjsme chteli rpecist Top
...
**/
	...
	//vlozim roota na stack
	tStackP stackP;
	SInitP(&stackP);
	SPushP(&stackP, *RootPtr);
	//vkladam potomky na stack a rusim vrchol zasobniku
	while(!SEmptyP(&stackP))
	{
		tBTNodePtr top = STopPopP(&stackP);
		if(top->LPtr) SPushP(&stackP, top->LPtr);
		if(top->RPtr) SPushP(&stackP, top->RPtr);
		free(top);
	}
	*RootPtr = NULL;
}
