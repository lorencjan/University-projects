...
class Cart:
    ...
    def log(self, msg):
        "a simple logger"
        if self.debug_lvl > 1:
            print("%d %s" % (jarvisenv.Jarvis.time(), self))
        if self.debug_lvl > 0:
            print("%d %s" % (jarvisenv.Jarvis.time(), msg))
    ...
    def finish_moving(self):
        self.log("moved to %s" % self.data)
        assert self.status == Status.Moving
        self.pos = self.data
        self.set_idle()
        #self.log("finished")

    def start_loading(self, cargo_req: CargoReq, slot):
        self.log("loading %s %s %s %s" % (cargo_req.src, cargo_req.content, cargo_req.weight, slot))
        self.check_idle()
    ...
    def finish_loading(self):
        ...
        self.log("loaded %s %s" % (self.pos, cargo_req.content))
    ...
    def start_unloading(self, slot):
        self.log("unloading_init %s" % self.pos)
        self.check_idle()
    ...
    def finish_unloading(self):
        ...
        self.log("unloaded %s %s" % (self.pos, cargo_req.content))
        assert self.status == Status.Unloading
        self.slots[self.data] = None
        self.set_idle()
        cargo_req.unload(self)
        self.log("unloading_finished %s %s" % (self.pos, cargo_req.content))
        return cargo_req
